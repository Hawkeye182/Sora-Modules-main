...
const { fetchv2 } = require('./test_fetchv2_direct.js');
...
async function testDirectPlaylistAccess() {
    ...
    try {
        ...
        const masterResponse = await fetchv2(masterM3u8Url, {
        ...
        const masterContent = typeof masterResponse === 'object' ? await masterResponse.text() : masterResponse;
        ...
        if (playlistMatches && playlistMatches.length > 0) {
            ...
            const directResponse = await fetchv2(directPlaylistUrl, {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36',
                'Accept': '*/*',
                'Origin': 'https://krussdomi.com',
                'Referer': 'https://krussdomi.com/',
            ...
            const directContent = typeof directResponse === 'object' ? await directResponse.text() : directResponse;
